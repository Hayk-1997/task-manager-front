  ...
  <div>
    ...
    >
      ...
      <label class="flex items-center cursor-pointer">
        <input v-model="user.is_available" type="checkbox" class="hidden" @change="toggle(user)" />
        <div class="w-12 h-6 bg-gray-300 rounded-full relative">
          <div
            :class="[
              'w-6 h-6 rounded-full absolute top-0 transition',
              user.is_available ? 'left-6 bg-green-500' : 'left-0 bg-red-500',
            ]"
          ></div>
        </div>
      </label>
...
const toggle = (user) => {
  store
    .dispatch('user/toggleAvailability', user)
    .then(() => {
      $toast.open({
